...
def f(x, *args):
    ...
    mask = np.int32(mask)
    mask = (np.arange(np.shape(X_tr)[1])+1)*mask-1
    mask = mask[mask>=0]
    XTR, XVA, YTR, YVA = train_test_split(X_tr[:,mask],
                                          y_tr, 
                                          test_size=0.1, 
                                          stratify=y_tr)
    estimator.fit(XTR, YTR)
    score = -calcAbsError(estimator, XVA, YVA)
    return score

bh = basinhopping(f, 
                  np.random.normal(0.5,0.1,np.shape(X_tr)[1]), 
                  niter=100, 
                  T=1.0, 
                  stepsize=0.5, 
